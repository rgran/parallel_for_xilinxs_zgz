...
//        for (auto &s : sout) cerr << s.str(); // salida threads
        ...
        char * execution_name = (char *) malloc(sizeof (char)*256);
         ...
         if(1)
	       sprintf(execution_name, "%s_%s_all.txt", pars->benchName, "MultiHap");
	    else
		   sprintf(execution_name, "%s_%s_%d_%d.txt",  pars->benchName, "MultiHap", nCPUs, nGPUs);
    ...
    void printHeaderToFile(ofstream &file) {
        file << "N. CPUs" << SEP << "N. GPUs" << SEP << "init ratio" << SEP << "Chunk" << SEP << "Time (ms)" << SEP << "fittime" << SEP << "%fitting" << SEP 
        #ifdef ENERGY
