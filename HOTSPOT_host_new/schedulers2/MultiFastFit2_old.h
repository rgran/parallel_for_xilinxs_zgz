...
__thread int myAccId = 0;   // identificador del acelerador controlado por el thread actual
...
public:
	...
	MySerialFilter_PROFILE(int b, int e, int ncpus, int ngpus, Params * para) : filter(true) {
		begin = b;
		end = e;
		nCPUs = ncpus;
		nGPUs = ngpus;
	...
	void * operator()(void *) {
		...
		if (begin < end) {
			...
			if (myAccId && AccStage[myAccId]>=0){				// vemos si es un thread con acelerador asociado
				...
				case 3: // Calculate model. Don't break return null to finish pipeline
		            ...
		            cerr << "FREQ MHz " << pars->frequency/1000000 << endl;
		            cerr << "THobj " << pars->thobjective << endl;
...
public:
	/*Class constructor, it only needs the first and last iteration indexes.*/
	MySerialFilter_STABLE(int b, int e, int ncpus, int ngpus, Params * para) : filter(true) {
		begin = b;
		end = e;
		nCPUs = ncpus;
		nGPUs = ngpus;
	...
	void * operator()(void *) {
		...
		if (begin < end) {
			...
			if (nAcc && !myAccId) { // asignar al thread un acelerador si hay libre y no tiene ya uno
				myAccId = nAcc--; // se puede modificar nAcc porque esta etapa es serie
			//	myAccId++; // el 0 está ocupado
			// quitar esto ANDRES
				set_thread_affinity_CORE(pthread_self(), nAcc);
                cerr << "** FPGA thread: " << nAcc+1 << endl;  // #1 already asigned
                chunk2tries=MAXCHUNK2TRIES;
			}
			else if(nCpu && !myCPUId)	
			{
			...
			if (myAccId && AccStage[myAccId]>=0){				// vemos si es un thread con acelerador asociado
                ...
                double NE;
                int auxchk;
                cerr << ">  FPGA stable " << myAccId << " Stage " << AccStage[myAccId] << endl;
				
			//	case 4: // Stable stage allways
					#ifdef TRACER
					char cad[256];
...
*/
       ...
       if(AccStage[myAccId] == 4)
       {
		 //  case 4:  // stable fase 
               ...
               body->OperatorGPU2(bundle->begin, bundle->end, bundle->type);
	 ...
	 //		   break;
	   ...
	   else
	   {
			...
			for(i=0; i<nGPUs-1; i++)	
			{
				...
				AccThreads[i]= std::thread(&B::OperatorGPU2,body,bundle->begin+(i*chunk), bundle->begin+((i+1)*chunk),i+1);
			}
		    cerr << "RUN THREAD FPGA "<< i << "chunk: " << bundle->begin+(i*chunk) << " - " << bundle->end << endl;
				
		    AccThreads[i]= std::thread(&B::OperatorGPU2,body,bundle->begin+(i*chunk), bundle->end,i+1);
			for(i=0; i<nGPUs; i++) 
			{
...
*/
	...
	void heterogeneous_parallel_for(int begin, int end, T* body, const size_t iters = 1){
		...
		global_chunk1 = pars->numgpus; // uno para cada FPGA core //ALIGNGPU((end-begin)/200);//(end-begin)/200;
		...
		if(nGPUs>100) body->OperatorGPU2(1,2,1);
