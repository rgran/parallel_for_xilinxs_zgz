...
public:
    ...
    void reset_iteration_space (int b, int e){
	    begin = b;
	    end = e;
...
#warning "unused code"
	    ...
	    for(size_t i = 0; i < iters; ++i) {
		    ...
		    serial_filter.reset_iteration_space(begin, end);
	    }
	     pipe.clear();
        } else {
            pipeline pipe;
...
//        for (auto &s : sout) cerr << s.str(); // salida threads
        ...
        if(1)
	       sprintf(execution_name, "%s_%s_all.txt", "AES", "MultiHap");
	    else
		   sprintf(execution_name, "%s_%s_%d_%d.txt", "AES", "MultiHap", nCPUs, nGPUs);
        ...
        file << nCPUs << "\t" << nGPUs << "\t"  << meanchunkGPU << "\t" <<  runtime << "\t"  <<  pars->chunkGPU_initratio << "\t"
        	#ifdef ENERGY
		<< en_CPU << "\t"  << en_FPGA << "\t"<< en_TOTAL<< "\t"
		
		#endif
		 << fitTime << "\t" << fitTime*100.0f/runtime << "\t" 
#ifdef PCM
    ...
    void printHeaderToFile(ofstream &file) {
        file << "N. CPUs" << SEP << "N. GPUs"  << SEP << "Chunk" << SEP << "Time (ms)" << SEP << "init ratio" << SEP 
        #ifdef ENERGY
			<< "CPU Energy(J)" << SEP << "FPGA Enegy(J)" << SEP << "Total Energy (J)" << SEP
		#endif
		<< "fittime" << SEP << "%fitting" << SEP 
#ifdef PCM
