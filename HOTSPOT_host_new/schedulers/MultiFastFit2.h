...
* Global variables
* **************************************************************************/
__thread chunk_size_t chunkCPU;      // chunk de CPU individual por thread
...
public:
 ...
 //~ _____           _       _  ______ _ _ _             ______           __ _ _ _             
//~ /  ___|         (_)     | | |  ___(_) | |            | ___ \         / _(_) (_)            
//~ \ `--.  ___ _ __ _  __ _| | | |_   _| | |_ ___ _ __  | |_/ / __ ___ | |_ _| |_ _ __   __ _ 
 //~ `--. \/ _ \ '__| |/ _` | | |  _| | | | __/ _ \ '__| |  __/ '__/ _ \|  _| | | | '_ \ / _` |
//~ /\__/ /  __/ |  | | (_| | | | |   | | | ||  __/ |    | |  | | | (_) | | | | | | | | | (_| |
//~ \____/ \___|_|  |_|\__,_|_| \_|   |_|_|\__\___|_|    \_|  |_|  \___/|_| |_|_|_|_| |_|\__, |
//~                                                                                       __/ |
//~  
/*My serial filter class represents the partitioner of the engine. This class selects a device and a rubrange of iterations*/
class MySerialFilter_PROFILE: public filter {
private:
	chunk_size_t begin;
	chunk_size_t end;
	int nCPUs;
...
public:
 int get_new_begin()
	...
	MySerialFilter_PROFILE(chunk_size_t b, chunk_size_t e, int ncpus, int ngpus, Params * para) : filter(true) {
		begin = b;
		end = e;
		nCPUs = ncpus;
		nGPUs = ngpus;
	...
	void * operator()(void *) {
		...
		if (begin < end) {
			...
			if (nAcc && !myAccId) { // asignar al thread un acelerador si hay libre y no tiene ya uno
                ...
                cerr << "** FPGA thread: " << myAccId << " AccStage[myAccId] " << AccStage[myAccId] << endl;
			...
			if (myAccId && AccStage[myAccId]>=0){				// vemos si es un thread con acelerador asociado
				...
				case 0: // FPGA WARMP UP
					...
					bundle->begin = begin;
					bundle->end = begin+global_chunk1;
					begin = bundle->end;
					bundle->type = myAccId;
				...
				case 3: // Calculate model. Don't break and continue with case:4
					...
					global_time1=time1;
					global_time2=time2;		
					global_chunk2=chunk2;
					//~ cerr << " CHK1 " <<  chunk1 << endl;
					//~ cerr << " CHK2 " << chunk2 << endl;
                ...
                */
				  if (anyFPGArunning) {
                    ...
                    for (int i=2; i < nGPUs+1; i++)
                    {
                        ...
                        maxGpuThroughput = MAX(maxGpuThroughput, gpuThroughput[i]);
					}
...
//~  _____           _       _  ______ _ _ _              _____ _        _     _      
//~ /  ___|         (_)     | | |  ___(_) | |            /  ___| |      | |   | |     
//~ \ `--.  ___ _ __ _  __ _| | | |_   _| | |_ ___ _ __  \ `--.| |_ __ _| |__ | | ___ 
//~  `--. \/ _ \ '__| |/ _` | | |  _| | | | __/ _ \ '__|  `--. \ __/ _` | '_ \| |/ _ \
//~ /\__/ /  __/ |  | | (_| | | | |   | | | ||  __/ |    /\__/ / || (_| | |_) | |  __/
//~ \____/ \___|_|  |_|\__,_|_| \_|   |_|_|\__\___|_|    \____/ \__\__,_|_.__/|_|\___

/*My serial filter class represents the partitioner of the engine. This class selects a device and a rubrange of iterations*/
class MySerialFilter_STABLE: public filter {
private:
	chunk_size_t begin;
	chunk_size_t end;
	int nCPUs;
...
public:
	/*Class constructor, it only needs the first and last iteration indexes.*/
	MySerialFilter_STABLE(chunk_size_t b, chunk_size_t e, int ncpus, int ngpus, Params * para) : filter(true) {
		begin = b;
		end = e;
		nCPUs = ncpus;
		nGPUs = ngpus;
	...
	void * operator()(void *) {
		...
		//		case 4: // Stable stage
                ...
                {
                    stopConditionModeOn = Acc_Stop_Condition();
#ifdef DEBUG
                    if(stopConditionModeOn) cerr << "  stopConditionModeOn Chunk FPGA= " << chunkGPU << " Begin = " << begin << ", End = " << end << " fg " << fG << endl;
#endif
					
				//	 AccsTH=totalAccThroughput();

                }
                   
					//float CPUsTH = avgCpuThroughput * nCPUs;
					chunk_size_t nextChunk = ALIGNFLOORGPU(chunkGPU);
					//~ cerr << "gpuThroughput[myAccId] : " << gpuThroughput[myAccId]  <<endl;
					//~ cerr << "CPUsTH        : " << CPUsTH  <<endl;
					//~ cerr << "chunkGPU     : " << chunkGPU << endl;
			
					//~ cerr << "nextChunk     : " << nextChunk << endl;
					//~ cerr << "(end - begin) : " << (end - begin) << endl;
					
					if(nCPUs==0 &&  (end - begin)/anyFPGArunning < chunkGPU)
					{
						// repartir entre las FPGAs, pero sólo una vez
						nextChunk = ALIGNFLOORGPU((end - begin)/anyFPGArunning);
						recordThroughput[myAccId] = false;
					...
					if (stopConditionModeOn) {
				         ...
				         float aproxTH = aproxAccTh_multi((end-begin)/anyFPGArunning);
				         //~ cerr << "aproxTH = " << aproxTH << endl;
					
                    nextChunk= (end-begin) * aproxTH / (aproxTH*anyFPGArunning + CPUsTH);
                    //~ cerr << "nextChunk = " << nextChunk << endl;
                    for (int k=0; k<3; k++)  // iterate to refine myChunk size
                    {
                      aproxTH = aproxAccTh_multi(nextChunk);
                      //~ cerr << "aproxTH = " << aproxTH << endl;
                         //~ cerr << "nextChunk = " << nextChunk << endl;
					  ...
					  nextChunk= 	(end-begin) * aproxTH / (aproxTH*anyFPGArunning + CPUsTH);
						...
						//nextChunk = calculateLastAccChunk_Andres((end-begin), CPUsTH, AccsTH, chunkGPU);
						nextChunk = ALIGNFLOORGPU(nextChunk);
						AccStage[myAccId]=-AccStage[myAccId];
						anyFPGArunning--;
						//~ cerr << "CPUsTH = " << CPUsTH << endl;
						//cerr << "AccsTH = " << AccsTH << endl;
						
					    cerr << "No hay suficiente. REMAIN: " << (end-begin) << " FPGACHUNK: " << nextChunk << endl;
						#ifdef DEBUG
						printf("New ACC SIZE: %d (begin %d THACC %f THCPUs %f)\n", nextChunk, begin, gpuThroughput[myAccId], CPUsTH);
						#endif
						
						if (aproxTH*5 < CPUsTH/nCPUs) { // la CPU es más rápida
							float maxGpuThroughput = gpuThroughput[myAccId];
						if(maxGpuThroughput > 0 && avgCpuThroughput > 0){
							fG = maxGpuThroughput/avgCpuThroughput;
							chunkCPU = ((end-begin < chunkGPU)? end-begin : chunkGPU) / (FREQFAC * fG) ;
							chunkCPU = MIN((float)chunkCPU, (float)(end-begin)/((float)nCPUs + fG));
						} else {
						...
						bundle->begin = begin;
						bundle->end = MIN(begin + chunkCPU, end);
						begin = bundle->end;
						bundle->type = CPU;
						#ifdef TRACER
						tracer.nodeStop(0, cad);
						#endif
				   	     cerr << "bundle FPGA -> faster CPU : " <<   bundle->end -bundle->begin << endl;
				     	bodies_C+=bundle->end-bundle->begin; 
				
						return bundle;
						}
						chunkGPU=nextChunk; // make chunk little?
					}
					if (nextChunk > 0) { // se le puede dar chunk alineado
						bundle->begin = begin;
						//~ cerr << "bundle begin : " << begin << endl;
						  //~ cerr << "bundle suma : " << begin + nextChunk << endl;
						  //~ cerr << "bundle end : " << end << endl;
						
						bundle->end = MIN(begin + nextChunk, end);
						begin = bundle->end;
						bundle->type = myAccId;
						#ifdef TRACER
						tracer.nodeStop(0, cad);
						#endif
					    //~ cerr << "bundle FPGA: " << bundle->begin << " - " << bundle->end  << endl;
				        bodies_F+=bundle->end-bundle->begin; 
						...
						tracer.nodeStop(0, cad);
						#endif
					//	  cerr << "bundle CPU: " << bundle->begin << " - " << bundle->end  << endl
						 bodies_C+=bundle->end-bundle->begin; 
...
//~ | ___ \             | | |    | | |  ___(_) | |           
//~ | |_/ /_ _ _ __ __ _| | | ___| | | |_   _| | |_ ___ _ __ 
//~ |  __/ _` | '__/ _` | | |/ _ \ | |  _| | | | __/ _ \ '__|
//~ | | | (_| | | | (_| | | |  __/ | | |   | | | ||  __/ |   
//~ \_|  \__,_|_|  \__,_|_|_|\___|_| \_|   |_|_|\__\___|_|
...
public:
	/*Class' constructor*/
	//template <class B>
	MyParallelFilter(B *b,int n) : filter(false) {
		body = b;
		nGPUs=n;
	}
...
cerr << "FPGA " << myAccId << " AccStage " << AccStage[myAccId] << endl;

    if(AccStage[myAccId] == 4)
       {
		 //  case 4:  // stable fase 
               start_tc = tick_count::now();
               body->OperatorGPU(bundle->begin, bundle->end, bundle->type);
               end_tc = tick_count::now();
               time = (end_tc - start_tc).seconds()*1000;
	 //		   break;
	   }
	   else
	   {
		   	std::thread AccThreads[nGPUs];
			long size = bundle->end-bundle->begin;
			if(size<nGPUs)
			{
			   start_tc = tick_count::now();
               body->OperatorGPU(bundle->begin, bundle->end, bundle->type);
			...
			{
			tbb::task_group g;
			tbb::task_arena limited_arena_prof(nGPUs, 1);        
		
			long chunk = size/nGPUs;
			//~ cerr << "SIZE = " << size << " CHUNK = " << chunk << endl;
			//*******************************************
			//*******************************************
			start_tc = tick_count::now();
			limited_arena_prof.execute([&]{
				for(int i=0; i<nGPUs-1; i++) g.run([&]{body->OperatorGPU(bundle->begin+(i*chunk), bundle->begin+((i+1)*chunk), i+1);});
				g.run([&]{body->OperatorGPU(bundle->begin+(i*chunk), bundle->end, i+1);});
			    g.wait();
			});
			end_tc = tick_count::now();
			...
			//~ for(i=0; i<nGPUs-1; i++)	
			//~ {
				//~ cerr << "RUN THREAD FPGA "<< i << " chunk: " << bundle->begin+(i*chunk) << " - " <<bundle->begin+((i+1)*chunk) << endl;
				//~ AccThreads[i]= std::thread(&B::OperatorGPU,body,bundle->begin+(i*chunk), bundle->begin+((i+1)*chunk),i+1);
			//~ }
		    //~ cerr << "RUN THREAD FPGA "<< i << "chunk: " << bundle->begin+(i*chunk) << " - " << bundle->end << endl;
				
		    //~ AccThreads[i]= std::thread(&B::OperatorGPU,body,bundle->begin+(i*chunk), bundle->end,i+1);
			//~ for(i=0; i<nGPUs; i++) 
			//~ {
				   //~ cerr << "  - JOIN THREAD FPGA "<< i << endl;
...
public:
	...
	//	cpuThroughputr=0.0;
		...
		for (int i=1; i<= pars->numgpus; i++) {  // th de todos los ACC = 0
			gpuThroughput[i] = 0;
			recordThroughput[i] = true;
			//if(pars->WARMUPFPGA) AccStage[i]=0;  else AccStage[i]=1;  // ANDRES init stage with/wout WARMUP
			AccStage[i]=4;
		} //ready for stable-fase
		recordThroughput[1] = false;
		if(pars->WARMUPFPGA) AccStage[1]=0;  else AccStage[1]=1;  // ANDRES init stage with/wout WARMUP
...
*/
	...
	void heterogeneous_parallel_for(int begin, int end, T* body, const size_t iters = 1){
		...
		if(stable_fase==0 && nGPUs)  // pipeline de exploración y cálculo del modelo
		//  __     __   ___              ___     ___      __        __   __        __     __       
		// |__) | |__) |__  |    | |\ | |__     |__  \_/ |__) |    /  \ |__)  /\  /  ` | /  \ |\ | 
		// |    | |    |___ |___ | | \| |___    |___ / \ |    |___ \__/ |  \ /~~\ \__, | \__/ | \| 
		{
		
		anyFPGArunning= 1; //ANDRES active Accs at the begining of pfor
		
		global_chunk1 = 1; //pars->numgpus; // uno para cada FPGA core //ALIGNGPU((end-begin)/200);//(end-begin)/200;
	...
	//	global_chunk22 = ALIGNGPU((end-begin)/20); //5%
		...
		cerr << "Create arena " << nCPUs << " + 1" <<endl;
		tbb::task_arena limited_arena(nCPUs + 1, 0);        
			/*Preparing pipeline*/
		pipeline pipe;
		MySerialFilter_PROFILE serial_filter(begin, end, nCPUs, nGPUs, pars);
		MyParallelFilter <T> parallel_filter(body,nGPUs);
		...
		if(!chunkCPU) chunkCPU=1;
	    /*Run the pipeline*/
	    cerr << "Pipeline profiling" << endl;
		nAcc=1;	
		
		//limited_arena.initialize(nCPUs + 1);
		cerr << "Exec arena" << endl;
		
		limited_arena.execute([&]{pipe.run(nCPUs + 1);});
		 cerr << "FIN Pipeline profiling" << endl;
		 begin=serial_filter.get_new_begin();
		 cerr << "begin-end: " << begin << " - " << end << endl;
		//pipe.run(nCPUs + nGPUs);
		pipe.clear();
		limited_arena.terminate();
		...
		/*Preparing pipeline*/
		pipeline pipe;
		MySerialFilter_STABLE serial_filter(begin, end, nCPUs, nGPUs, pars);
		MyParallelFilter<T> parallel_filter(body,nGPUs);
		pipe.add_filter(serial_filter);
		pipe.add_filter(parallel_filter);
		//chunkCPU = chunkGPU * 0.2;
		...
		/*Run the pipeline*/
		//if (nCPUs>200) body->OperatorGPU(1,100,1);
		/*Run the pipeline*/
