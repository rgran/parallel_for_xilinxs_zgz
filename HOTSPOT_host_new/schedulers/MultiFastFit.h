			   ...
			   case 22: // chunk pre-stable
				...
				//	 AccsTH=totalAccThroughput();
					...
					if (nextChunk > 0) { // se le puede dar chunk alineado
				        ...
				        bodies_F+=bundle->end-bundle->begin; 
				        meanchunkGPU+=bundle->end-bundle->begin;
                    num_chunkGPU++;
						return bundle;
	...
	bool fexists(const char *filename)
{
  ...
  return (bool)ifile;
	...
	void saveResultsForBench(){
	    ...
	    if(1)
	       sprintf(execution_name, "%s_%s_all.txt", pars->benchName, "FastFit");
	    else
		   sprintf(execution_name, "%s_%s_%d_%d.txt",  pars->benchName, "FastFit", nCPUs, nGPUs);
		...
		cout << "dattos chunk medio: " << meanchunkGPU << " / " << num_chunkGPU << endl;
        meanchunkGPU/=num_chunkGPU;
		cout << "TIEMPO TOTAL: " << runtime << endl;
		cout << "chunk modelo: " << chunkGPU_modelo << endl;
		cout << "chunk medio: " << meanchunkGPU << endl;
		
		file << nCPUs << "\t" << nGPUs << "\t"  << meanchunkGPU << "\t" <<  runtime << "\t" << pars->thobjective << "\t"
		
		#ifdef ENERGY
		<< en_CPU << "\t"  << en_FPGA << "\t"<< en_TOTAL<< "\t"
		
		#endif
		<< bodies_F*100/(bodies_F+bodies_C) << "\t"  << chunkGPU_modelo << "\t"
		#ifdef PCM
