...
#         for freq_cpu in 1200; do
	  ...
	  echo ./spmm_fast.sh -n ${n_veces} -m ${input_matrix} -d ${scheduler_type} -c ${core} -p ${acc} -k ${chunk_size} -i -u ${freq_cpu} -v ${freq_fpga} -s "${out_file}"
	  ./spmm_fast.sh -n ${n_veces} -m ${input_matrix} -d ${scheduler_type} -c ${core} -p ${acc} -k ${chunk_size} -i -u ${freq_cpu} -v ${freq_fpga} -s "${out_file}"
