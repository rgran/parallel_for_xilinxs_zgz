...
public:
	...
	/*Sets the start mark of energy and time*/
		void startTimeAndEnergy(){
			...
			#ifdef ENERGY
				energy=true;
				sample1=energy_meter_init(5, 0  /*0=no debug*/);  // sample period 100 miliseconds
		...
		/*Sets the start mark of energy and time*/
		void startTimeAndEnergy(bool energia, int ms, int debug){
			printf("starting time energy\n");
		#ifdef ENERGY
		energy=energia;
		if(energy)
		{
	  		sample1=energy_meter_init(ms, debug  /*0=no debug*/);  // sample period 100 miliseconds
	  		//power_meter_idle(sample1); //get idle power
	  		energy_meter_start(sample1);  // starts sampling thread
		}
		#endif
	...
	void endTimeAndEnergy(){
		...
		if(energy)
		{
	  		...
	  		energy_meter_totals(sample1,&en_CPU,&en_FPGA,&en_TOTAL);
