...
int calculateLogarithmicModelv2(float *x, float *y, int numberPoints, int last_point) {
    ...
    if (calculatedChunk==1) return x[last_point];
#endif
    //return 100000;
...
public:
    ...
    void * operator()(void *) {
        ...
        #ifdef DEBUG
         cerr << "** Begin: " << begin << " -> " << end << endl;
         cerr << "** myAccId: " << myAccId << endl;
         cerr << "** lastmeasurements: " << lastmeasurements << endl;
         cerr << "** stopConditionModeOn: " << stopConditionModeOn << endl;
         cerr << "** explorationPhase: " << explorationPhase << endl;   
         cerr << "** anyFPGArunning: " << anyFPGArunning << endl;   
                    ...
                    if (x[lastmeasurements] < lastchunkGPU && x[measurements-1] < lastchunkGPU && !stopConditionModeOn) {  // nueva condición
                        ...
                        else
                        {
                            ...
                            if (  (filledOK &&
                                ...
                                if (  (filledOK)) cerr << "FILLED OK\n";
                                cerr << "Interation space: " << iterationSpace << endl;
                                cerr << "último: " << x[lastmeasurements] << endl;
...
#ifdef DEBUG
                            cerr << "******** Fin espacio iteraciones ********* "<< endl;
#endif
        return NULL;
    } // end operator
...
public:
    ...
    void heterogeneous_parallel_for(int begin, int end, T *body) {
        ...
        if(veryfirsttime)
        {
            ...
            minChunkCPU = min(10, (int)((end-begin)/100));
