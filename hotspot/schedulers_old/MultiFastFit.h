 ...
 *  1.- first chunk
 *  2.- second chunk
 * 22.- second chunk, second try
 *  3.- calculate model
 * 33.- calculate model second try
 *  4.- stable
 * */
int global_chunk1, global_chunk2;
		...
		if (begin < end) {
			...
			if (myAccId && AccStage[myAccId]>=0){				// vemos si es un thread con acelerador asociado
				...
				case 3: // Calculate model. Don't break and continue with case:4
					...
					if (NE <= 0 || auxchk <= 0) { // Second chunk, 2nd try
						AccStage[myAccId]=22;
						chunk2 = (end-begin)/10;
						recordThroughput[myAccId] = false;
						bundle->begin = begin;
				...
				case 33: // Calculate model, 2nd try. Don't break and continue with case:4
					if(AccStage[myAccId]==33)
					{
					recordThroughput[myAccId] = true; // this is for good
					AccStage[myAccId]=4;
...
if(bundle->type==nAcc+1)
{
    std::thread AccThreads[nAcc];
	long size = bundle->end-bundle->begin;
	long chunk = size/nAcc;
	
	tick_count start_tc = tick_count::now();
	for(int i=0; i<nAcc-1; i++)	AccThreads[i]= std::thread(body->OperatorGPU,bundle->begin+(i*chunk), bundle->begin+((i+1)*chunk),i+1);
	AccThreads[i]= std::thread(body->OperatorGPU,bundle->begin+(i*chunk), bundle->end,i+1);
    for(int i=0; i<nAcc; i++) AccThreads[i].join();
...
*/
      ...
      //  lastcpuThroughput=cpuThroughput;
        ...
        cerr << "=== Chunk CPU: " << bundle->end - bundle->begin << " BEGIN: " << bundle->begin << " TH: " << cpuThroughputr /*((tg5-tg1)/1000000.0) */ << endl;
#endif
...
*/
	...
	void heterogeneous_parallel_for(int begin, int end, T* body, const size_t iters = 1){
		...
		global_chunk1 = ALIGNGPU(pars->INITCHUNK);//(end-begin)/200;
		global_chunk2 = ALIGNGPU((end-begin)/20);
