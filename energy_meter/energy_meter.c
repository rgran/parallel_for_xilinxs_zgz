...
void energy_meter_totals(struct energy_sample *sample1, float *CPU, float *FPGA, float *TOTAL)
{
	*CPU=subtotal_em((*sample1),0,PSD_rails)/1000.0;
	*FPGA=subtotal_em((*sample1),PSD_rails,PLD_rails)/1000.0;
	*TOTAL=subtotal_em((*sample1),0,PLD_rails)/1000.0;
...
void energy_meter_printf(struct energy_sample *sample1, FILE * fout)
{
	...
	fprintf(fout,"** Processing System Domain rails ********** \n");
	for (i=0; i<PSD_rails; i++) {
	...
	for (i=PSD_rails; i<PLD_rails; i++) {
	fprintf(fout,"%14s = %8.3lf J ",rails[i],sample1->energy[i]/1000.0);
	if((i+1)%3==0) fprintf(fout,"\n");
	}
	fprintf(fout,"\n\n");
	
	fprintf(fout,"Processing System Domain ENERGY = %lf J\n",subtotal_em((*sample1),0,PSD_rails)/1000.0);
	fprintf(fout,"Programmable Logic Domain ENERGY = %lf J\n",subtotal_em((*sample1),PSD_rails,PLD_rails)/1000.0);
	fprintf(fout,"TOTAL ENERGY = %lf J\n",subtotal_em((*sample1),0,PLD_rails)/1000.0);
	fprintf(fout,"\n");	
	

	fprintf(fout,"Processing System Domain POWER = %lf W\n",subtotal_em((*sample1),0,PSD_rails)/1000.0/((double)res.tv_sec+ (double)res.tv_nsec/1000000000.0));
	fprintf(fout,"Programmable Logic Domain POWER = %lf W\n",subtotal_em((*sample1),PSD_rails,PLD_rails)/1000.0/((double)res.tv_sec+ (double)res.tv_nsec/1000000000.0));
		          ...
		          fprintf(fout,"\n");
			          fprintf(fout,"** PL rails *************************\n");
				          for (i=PSD_rails; i<PLD_rails; i++) {
					          ...
					          fprintf(fout,"\n");
						          fprintf(fout,"PS ENERGY= %lf J\n",subtotal_em((*sample1),0,PSD_rails));
							          fprintf(fout,"PL ENERGY= %lf J\n",subtotal_em((*sample1),PSD_rails,PLD_rails));
								          fprintf(fout,"TOTAL ENERGY= %lf J\n",total_em((*sample1)));
	fprintf(fout,"\n");
}
//-------------------------------------------------------------------
